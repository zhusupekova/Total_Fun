...
- Ассеты: `assets/` принимает GLB (`arena.glb`, `player.glb`, `ball.glb`); сейчас лежат плейсхолдеры и визуальные референсы в корне/`docs/`.
...
## Референсы и ассеты
- Визуальные рефы: `ref_*.jpeg/png` в корне (арена, персонажи, UI).
...
## Демо чек-лист Stage 1
Что можно показать на текущем стенде:
...
- Демонстрация: 4 слота игроков (1 управляемый + 3 бота); мяч скользит по плоскости, отражается от стен и игроков, меняет скорость, не подпрыгивает; персонажи движутся только по своим сторонам без вращения.
- Ожидаемое: стабильный FPS, предсказуемые отражения, тень под мячом. Сети и тяжёлых эффектов здесь нет.
